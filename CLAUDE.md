...
## テストファイル肥大化防止 — レビュー・Issue 分解の観点

実装規約は `AGENTS.md`「テストファイル肥大化防止ルール」「テスト実行時間短縮ルール」に集約している。本セクションはレビューと Issue 分解時に追加で確認すべき観点を補足する。

### Issue 分解時の方針

テスト戦略を Issue に書く際は次を必ず明示すること:

- **新規ファイルが必要かどうか** — 既存の `<target>_test.dart` に追加できる場合、受け入れ基準に「`test/path/to/<target>_test.dart` の `<group 名>` group に追加」と書く
- **既存ファイルへの追加で済む Issue で「新規 `xxx_test.dart` を追加」と書かない** — 分解段階で安易に新ファイルを増やさない
- **新規ファイルが本当に必要なときのみ** 「新規 `<file>` を追加」と明示する（基準: 対象自体が新規、または既存ファイルが 2,000 行を超え観点が分離可能）

### PR レビュー時に **must fix** として差し戻す観点

次のパターンが見つかった場合、マージ前に既存ファイルへの統合・最適化を要求する:

- 既存 `<target>_test.dart` がある状態で新規 `<target>_xxx_test.dart` が追加されている
- 1 group しか持たない `xxx_test.dart` の追加（既存ファイルへ group 追加で済むはず）
- 同じ fake クラスが 2 ファイル以上にインライン重複している（`test/helpers/` への抽出が必要）
- 単一テストケースのために新ファイルを作っている
- 横断的観点（コントラスト・i18n・テーマ走査）が対象別に分割されて複数ファイルになっている
- 純粋ロジックのテストが不必要に `testWidgets` で書かれている
- `pumpAndSettle` が「アニメーション完了の確認」以外の目的で使われている

### 統合・分割の判断基準

レビュー中に既存ファイルの統合・分割を判断するときは、次の数値基準を用いる:

**統合候補（既存ファイルへ吸収せよ）:**
- **同一対象に対する別ファイルが存在し**、かつどちらかが 1 group のみ + 100 行未満（例: `xxx_config_test.dart` + `xxx_test.dart`）
- 同一画面に対する複数の小型補助テストファイル（`xxx_config_test.dart` + `xxx_error_test.dart` 等）
- 同一観点を別々のファイルで断片化（`gift_contrast_test.dart` + `nicoad_contrast_test.dart` 等）

**統合候補ではない（小型でも単独維持してよい）:**
- 別々の domain ターゲット（pure function / pure class）に対する独立したテスト（例: `nico_icon_url_test.dart`, `lv_parser_test.dart` のように対象が他ファイルと重複しないもの）
- 1 group + 小規模であっても、関連する受け皿ファイルが無ければ統合先がないので単独で OK

**物理分割を許容する条件:**
- 1 ファイル 2,000 行超
- テスト視点が明確に複数（例: スクリーンの音声機能と検索機能）
- group ネストが 3 階層以上必要

数値基準を満たさない統合・分割提案はスコープ外として OPTIONAL に格下げし、別 PR を推奨する（feature PR にリファクタリングを混ぜない）。

### ルール適用範囲（forward-looking）

本セクションのルールは **新規追加・新規変更されるテスト** に対して適用する。既存テストの retrofit（過去のルール違反のクリーンアップ）は別 Issue で段階対応する方針とする。レビューで既存テストに違反を発見した場合は OPTIONAL として記録するに留め、当該 PR 内での修正は要求しない。
