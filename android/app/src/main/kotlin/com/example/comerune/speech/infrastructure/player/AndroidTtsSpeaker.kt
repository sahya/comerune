            ...
            when (event) {
                ...
                AudioFocusGuard.FocusEvent.LOSS_TRANSIENT_CAN_DUCK -> {
                    ...
                    if (cont != null && cont.isActive) {
                        // Issue #964: TOCTOU defence — a listener callback
                        // can resume between the isActive check and
                        // resume() below. Issue #966: surface as the
                        // sealed FocusLost sub-type so the controller can
                        // dispatch on type.
                        try {
                            cont.resume(
