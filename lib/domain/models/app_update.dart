...
/// GitHub Releases から解決したバージョン情報。
///
/// アプリは起動時に GitHub Releases API から最新の安定リリース 1 件を取得し、
/// `tag_name` を [latest]、本文中の `<!-- min-supported: X.Y.Z -->` マーカーを
/// [minSupported]、`html_url` を [releaseUrl] にマッピングする。
class RemoteVersionManifest {
  ...
  /// 最新版。解析できなければ null（その場合は任意更新通知を出さない）。
  final SemanticVersion? latest;

  /// この版未満は強制更新対象。マーカー不在／不正なら `0.0.0` とし、
  /// 結果的に誰も強制ブロックされない（安全側）。
  final SemanticVersion minSupported;

  /// 更新を案内する公開ページ URL。null の場合あり。
...
class UpdateCheckResult {
  ...
  /// GitHub Releases から情報を取得し判定まで到達できたか。
