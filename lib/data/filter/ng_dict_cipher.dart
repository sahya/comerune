...
/// offset 20 mac       32 bytes  = HMAC-SHA256(key, magic||salt||ciphertext)
/// offset 52 ciphertext N bytes  = plaintext XOR keystream(key, salt)
/// ```
///
...
Uint8List decryptNgDict(Uint8List blob) {
  ...
  final Uint8List expectedMac = _mac(key, salt, ciphertext);
  if (!_constantTimeEquals(mac, expectedMac)) {
    throw const NgDictCipherException('integrity check failed');
  }

  final Uint8List keystream = _keystream(key, salt, ciphertext.length);
