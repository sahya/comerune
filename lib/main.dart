...
import 'data/app_update/app_version_repository.dart';
...
Future<void> main() async {
  ...
  // GitHub リリース連動のバージョン更新通知・強制更新。
  // 配信エンドポイント URL はビルド時注入（--dart-define）。未設定の
  // 公開クローン／フォークでは AppVersionRepository.fetch() が null を
  // 返し、機能全体が静かに no-op になる（CLAUDE.md 2 段フォールバック）。
  // 対象は Android のみ。それ以外は VersionUpdateChecker が none を返す。
...
class ComeruneApp extends StatefulWidget {
  ...
  /// GitHub リリース連動の更新判定。起動時フローと設定画面の手動確認で
  /// 共有する。null（最小テストハーネス等）の場合は更新確認を行わない
...
class _ComeruneAppState extends State<ComeruneApp> {
  ...
  ///
  /// fail-open: 非対象 OS・配信元未設定・取得失敗時は何も出さない。
