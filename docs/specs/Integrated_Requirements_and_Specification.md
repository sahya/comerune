...
- `TimelineStore`
  - コメント保持（リングバッファ、上限は過去コメント取得件数設定 + 新着バッファ 5000 件。既定: **500件取得 + 5000件バッファ = 最大5500件保持**）、重複排除、UIへ流す
- `SpeechEngine`（抽象）
...
- 入口WSで「イベント型が変わる」可能性を考慮し、未知イベントでも raw JSON をデバッグログに残し、URL検出のフォールバックを常に働かせる
  - raw JSON のログ出力時は以下のマスキングを適用する:
    - 機密キー（`token`, `cookie`, `session`, `auth`, `credential`, `secret` 等）の値を `"***"` に置換する。キー名の照合は大文字小文字を無視し、ネストされたオブジェクト・配列にも再帰的に適用する
    - URL値は既存ルール（`scheme://host/path` のみ、クエリパラメータ除去）を適用する
    - 上記で捕捉できない未知の機密情報に備え、値が40文字以上の文字列は末尾を `"…"` で切り詰める（トークン様の長い文字列の全文露出を防ぐ）
...
### 8.2 過去コメント初期ロード
- 直近500件を既定とする（設定で変更可能: 100 / 500 / 1000 / 全部）
- 明示的に `100` を保存していた既存ユーザーは `100` を維持する（後方互換）。未保存・未知の値は新デフォルト `500` にフォールバックする
- NDGRの backward/segment を辿って過去を補完する（参考実装に準拠）
- 設定値に応じて TimelineStore のリングバッファ上限は「取得件数 + 新着バッファ 5000 件」で連動する（§5.1参照）。取得直後の過去コメントが新着流入で即座にトリムされないようにバッファを確保する設計。バッファを超えた分の新着は古いもの（最古の過去コメント）から順に破棄される
- 「全部」選択時はバッファ上限を十分大きな値（例: 10000件取得 + 5000件バッファ = 最大15000件）に設定し、取得可能な過去コメントをすべて段階的に取得する。ただしバッファ上限を超えた分は古いものから破棄されるため、厳密な全件保持ではない
...
- v1.2では `chat` キーを固定の抽出対象とする
  ...
  - `chat` キーが存在しない受信メッセージは、コメント一覧にシステム行「legacy: 未対応フォーマット」を表示する（デバッグログには raw JSON を出力する）。受信メッセージごとに1行追加し、リングバッファで管理する。読み上げ対象外とする
