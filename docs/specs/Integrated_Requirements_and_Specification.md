...
- `TimelineStore`
  - コメント保持（リングバッファ、既定上限: **100件**）、重複排除、UIへ流す
- `SpeechEngine`（抽象）
...
- 初期ロード件数は TimelineStore のリングバッファ上限（§5.1: 100件）以内に収まるよう取得する。初期ロード完了後にリアルタイムコメントが流入しても最新コメントが保持されるよう、リングバッファは初期ロード後もスライドして動作する（古いものから順に破棄）
