...
> 補足: `8{3,}` の読み上げは「3.5.6 辞書置換」のユーザー辞書に委譲する。
> ハードコードで圧縮するとユーザーが辞書で別の読み（既定: `ぱちぱちぱち`）
> に変えても上書きされず反映されない不具合が発生するため、この行は意図的
> に圧縮対象から外している。
...
class DefaultCommentNormalizer : CommentNormalizer {
    ...
    override fun normalize(raw: RawComment, settings: SpeechSettings): NormalizedComment {
        ...
        // 8{3,} の読み上げは下の dictionaryRules 段で処理する（ユーザー辞書尊重）。
